...
public:
	explicit NormaliseExprVisitor(CompilerInstance *CI, FunctionFilter const &functionFilter)
			: astContext(&(CI->getASTContext())), functionFilter(functionFilter)
	{
	...
	bool TraverseBinaryOperator(BinaryOperator *binop)
	{
		...
		case clang::BinaryOperator::Opcode::BO_Comma:
			return TraverseCommaOperator(binop);
		case clang::BinaryOperator::Opcode::BO_LAnd:
			return TraverseLAndOperator(binop);
		case clang::BinaryOperator::Opcode::BO_LOr:
			return TraverseLOrOperator(binop);
		default:
			return RecursiveASTVisitor<NormaliseExprVisitor>::TraverseBinaryOperator(binop);
		}
	}
...
public:
	...
	{
		if (!rewritten)
		{ // early exit if nothing has been rewritten so as to not try to make a buffer etc.
			return;
		}
...
class FrontendActionDataFactory : public clang::tooling::FrontendActionFactory
{
	std::tuple<typename std::decay<V>::type...> args;

	template <std::size_t... I>
	std::unique_ptr<FrontendAction> create_impl(std::index_sequence<I...>)
	{
