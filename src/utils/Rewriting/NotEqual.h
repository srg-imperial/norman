...
namespace rewriting {
	template <typename T, typename U> NotEqual<std::decay_t<T>, std::decay_t<U>> make_neq(T&& operand1, U&& operand2) {
		return {std::forward<T>(operand1), std::forward<U>(operand2)};
	}
} // namespace rewriting

template <typename T, typename U> struct fmt::formatter<rewriting::NotEqual<T, U>> {
	template <typename ParseContext> constexpr auto parse(ParseContext& ctx) { return ctx.begin(); }

	template <typename FormatContext> auto format(rewriting::NotEqual<T, U> const& node, FormatContext& ctx) {
