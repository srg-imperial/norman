...
namespace rewriting {
	...
	LogicalAnd<std::decay_t<T>, std::decay_t<U>, std::decay_t<V>...> make_land(T&& operand1, U&& operand2,
	                                                                           V&&... operands) {
...
template <typename... V> struct fmt::formatter<rewriting::LogicalAnd<V...>> {
	template <typename ParseContext> constexpr auto parse(ParseContext& ctx) { return ctx.begin(); }

	template <typename FormatContext> auto format(rewriting::LogicalAnd<V...> const& node, FormatContext& ctx) {
...
private:
	...
	template <typename Out, typename... W> Out format_one(Out out, rewriting::LogicalAnd<W...> const& expr) {
		return fmt::format_to(out, FMT_COMPILE("{}"), expr);
	}
};
